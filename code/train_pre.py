...
from transformers import get_linear_schedule_with_warmup, AutoTokenizer, AutoModel, AutoModelForCausalLM

from dataset_process import PreprocessedDataset, PreprocessedPreDataCollator
...
if __name__ == '__main__':
    ...
    train_dataset = PreprocessedDataset(args.dataset, 'train', 'pre', args.debug)
    valid_dataset = PreprocessedDataset(args.dataset, 'valid', 'pre', args.debug)
    test_dataset = PreprocessedDataset(args.dataset, 'test', 'pre', args.debug)
    data_collator = PreprocessedPreDataCollator(tokenizer, pad_entity_id=len(entity2id), prompt_tokenizer=text_tokenizer, debug=args.debug, max_length=args.max_length, entity_max_length=args.entity_max_length, prompt_max_length=args.prompt_max_length, use_amp=args.fp16)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        valid_metrics = {f'valid/{k}': v / gathered_valid_report.get('count', 1) for k, v in gathered_valid_report.items() if k != 'count'}
    ...
    logger.info("***** Running Test on Best Model *****")
    state_dict = torch.load(best_model_path, map_location='cpu')
    unwrapped_model = accelerator.unwrap_model(crs_model)
    unwrapped_model.prompt_encoder.load_state_dict(state_dict)
    logger.info(f"Loaded best model state from {best_model_path} for testing.")
    ...
    test_report_raw = test_evaluator.report()
    gathered_test_report = {k: accelerator.gather(v).sum().item() for k, v in test_report_raw.items()}
    test_metrics = {f'test/{k}': v / gathered_test_report.get('count', 1) for k, v in gathered_test_report.items() if k != 'count'}
    test_metrics['test/loss'] = np.mean(test_loss)
    logger.info(f"Final Test Report on Best Model: {test_metrics}")
    if run: run.log(test_metrics)
